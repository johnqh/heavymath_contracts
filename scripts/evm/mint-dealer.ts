...
async function main() {
  ...
  const priceHash = await dealerNFT.write.setMintPrice([0n], {
  ...
  const tokenId = await dealerNFT.read.tokenOfOwnerByIndex([
  ...
  const permHash = await dealerNFT.write.setPermissions(
    [tokenId, 255n, [255n]],
    { account: deployer.account }
  );
  console.log('Permission tx:', permHash);
  const permReceipt = await publicClient.waitForTransactionReceipt({
    hash: permHash,
  });
